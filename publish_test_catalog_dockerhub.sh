...
opm index add \
  ...
  --tag docker.io/$DOCKER_HUB_REPOSITORY/release-operator-index:25.1 \
  --generate
docker build -f index.Dockerfile -t docker.io/$DOCKER_HUB_REPOSITORY/release-operator-index:25.1 .
docker push docker.io/$DOCKER_HUB_REPOSITORY/release-operator-index:25.1
